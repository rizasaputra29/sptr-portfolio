...
>(
  (
    ...
    // Removed colorTweenRef as we will use CSS classes
    ...
    useLayoutEffect(() => {
      const ctx = gsap.context(() => {
        ...
        // Removed initial GSAP color set to rely on Tailwind classes
    ...
    const isButtonVisible = !headless || open;
    const buttonStyle: React.CSSProperties = {
      opacity: isButtonVisible ? 1 : 0,
      pointerEvents: isButtonVisible ? "auto" : "none",
      transition: "opacity 0.2s ease",
      marginLeft: headless ? "auto" : undefined,
    };

    const logoStyle: React.CSSProperties = {
      display: headless ? "none" : "flex",
    };
    ...
    return (
      ...
      >
        ...
        >
          ...
          <header
            className="staggered-menu-header absolute top-0 left-0 w-full flex items-center justify-between p-[2em] bg-transparent pointer-events-none z-70"
          ...
          <aside
            ...
            ref={panelRef}
            className="staggered-menu-panel absolute top-0 right-0 h-full bg-white dark:bg-black flex flex-col p-[6em_2em_2em_2em] overflow-y-auto z-10 backdrop-blur-md transition-colors duration-300"
              ...
              >
                {items && items.length ? (
                  items.map((it, idx) => (
                    ...
                    >
                      <a
                        className="sm-panel-item relative text-black dark:text-white font-semibold text-[2.5rem] cursor-pointer leading-none tracking-[-1px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]"
                        href={it.link}
                  ...
                  <li
                    ...
                    aria-hidden="true"
                  >
                    <span className="sm-panel-item relative text-black dark:text-white font-semibold text-[2.5rem] cursor-pointer leading-none tracking-[-1px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]">
                      <span className="sm-panel-itemLabel inline-block origin-[50%_100%] will-change-transform">
                        No items
                      </span>
                    </span>
                    ...
                    {socialItems.map((s, i) => (
                      <li key={s.label + i} className="sm-socials-item">
                        <a
                          ...
                          className="sm-socials-link text-[1.2rem] font-medium text-[#111] dark:text-zinc-200 no-underline relative inline-block py-0.5 transition-[color,opacity] duration-300 ease-linear"
                        >
                          {s.label}
                        </a>
                      </li>
        ...
        <style>{`
  .sm-scope .staggered-menu-wrapper { position: relative; width: 100%; height: 100%; z-index: 60; }
  ...
  .sm-scope .sm-panel-item { position: relative; font-weight: 600; font-size: 2.5rem; cursor: pointer; line-height: 1; letter-spacing: -1px; text-transform: uppercase; transition: background 0.25s, color 0.25s; display: inline-block; text-decoration: none; padding-right: 1.4em; }
